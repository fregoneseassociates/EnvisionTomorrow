...
CleanUp:
    ...
    Public Sub EnvisionLyrFieldTrackingCheck(ByVal pTable As ITable)
        ...
        For intRow = 0 To intRowCount - 1
            ...
            strFieldName = rowTemp.Value(pFieldTable.FindField("FIELD_NAME"))
            strFieldAlias = rowTemp.Value(pFieldTable.FindField("FIELD_ALIAS"))
            strFieldType = rowTemp.Value(pFieldTable.FindField("FIELD_TYPE"))
            intFieldWidth = rowTemp.Value(pFieldTable.FindField("FIELD_WIDTH"))
            intFieldDecimal = rowTemp.Value(pFieldTable.FindField("FIELD_DECIMAL"))
            intCalc = rowTemp.Value(pFieldTable.FindField("CALC_BY_ACRES"))
            strcalcFieldName = rowTemp.Value(pFieldTable.FindField("CALC_FIELD_NAME"))
            strUseField = rowTemp.Value(pFieldTable.FindField("USE"))
...
CleanUp:
    ...
    Public Function OpenEnvisionExcel() As Boolean
        ...
        Try
            ...
            Next
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Opeing Envision Excel File Error", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly, False)
            OpenEnvisionExcel = False
            CloseEnvisionExcel()
            GoTo CleanUp
        End Try
    ...
    Public Function LoadEnvisionEditLayer(ByVal strPath As String, ByVal strName As String, ByVal blnPrompt As Boolean) As Boolean
        ...
        Try
            If Not blnLayerFound Then
                pWksFactory = New FileGDBWorkspaceFactory
                pFeatWks = pWksFactory.OpenFromFile(strPath, 0)
                ...
                For intRow = 1 To 35
                    Try
                        ...
                        If strFldCellValue.Contains(",") Then
                            strFldCellValue = CStr(strFldCellValue.Split(",")(0))
                        End If
                    Catch ex As Exception
                        strFldCellValue = ""
                    End Try
    ...
    Public Function Write2EnvisionExcelFile() As Boolean
        ...
        Else
            ...
            Try
                ...
                m_appEnvision.StatusBar.Message(0) = "Retrieving current Scenarion tab: " & "SCENARIO" & m_intEditScenario.ToString
                Try
                    shtScenario = m_xlPaintWB1.Sheets("SCENARIO" & m_intEditScenario.ToString)
                    If Not TypeOf m_xlPaintWB1.Sheets("SCENARIO" & m_intEditScenario.ToString) Is Microsoft.Office.Interop.Excel.Worksheet Then
                ...
                'MAKE SURE THE PROCESSING ROW COUNT DOES NOT EXCEED 50
                If intRowCount > 50 Then
                    intRowCount = 50
                End If
                ...
                'BUILD LIST OF COUlMNS TO PROCESS
                If intScTabFieldRow > -1 Then
                    ...
                    'POPULATE THE EXCEL FROM DATA GRID....ASSUMES THE DEVELOPMENT TYPE ORDER REMAINS STATIC
                    m_appEnvision.StatusBar.Message(0) = "Writing values to Scenarion tab: " & "SCENARIO" & m_intEditScenario.ToString
                            ...
                            Try
                                ...
                                If Not strFieldName = "" Then
                                    If tblsc.FindField(strFieldName) >= 0 Then
                                        ...
                                        dblValue = RoundNumber(dblValue, 6)
...
CleanUp:
    ...
    Public Function WriteValues(ByVal pPoint As IPoint, ByVal pPolygon As IPolygon, ByVal pPolyline As IPolyline) As Boolean
        ...
        If Not pPoint Is Nothing Or Not pPolygon Is Nothing Or Not pPolyline Is Nothing Then
            ...
            pCursor = m_pEditFeatureLyr.FeatureClass.Update(pSpatFilter, True)
        ...
        If Not m_envSelect Is Nothing Then
            mxDoc.ActiveView.PartialRefresh(5, Nothing, m_envSelect.Envelope)
        End If
        m_envSelect = pREnv
        ...
        If pFeatSelection.SelectionSet.Count > 0 Then
            ...
            Try
                ...
                Do While Not pFeat Is Nothing
                    ...
                    'SUBTRACT DEVD AVRES FROM EXISTING LU PAINT TABLE
                    If strDevType.Length > 0 then
                    If Not m_tblExistingLU Is Nothing Then
                        strEX_LU = ""
                        Try
                            If pFeatClass.FindField("EX_LU") >= 0 Then
                                strEX_LU = pFeat.Value(pFeatClass.FindField("EX_LU"))
                                If strEX_LU.Length > 0 Then
                                    For intEX_LUrec = 1 To m_tblExistingLU.RowCount(Nothing)
                                        rowEX_LU = m_tblExistingLU.GetRow(intEX_LUrec)
                                        If rowEX_LU.Value(m_tblExistingLU.FindField("EX_LU")) = strEX_LU Then
                                            dblEX_LUAcres = rowEX_LU.Value(m_tblExistingLU.FindField("Sum_DEVD_ACRE"))
                                            rowEX_LU.Value(m_tblExistingLU.FindField("Sum_DEVD_ACRE")) = dblEX_LUAcres - dblDevelopedAcres
                                            rowEX_LU.Store()
                                            rowEX_LU = Nothing
                                            Exit For
                                        End If
    ...
    Public Sub WriteAddDevType()
        ...
        If pFeatSelection.SelectionSet.Count > 0 Then
            ...
            Try
                ...
                Do While Not pFeat Is Nothing
                    intCount = intCount + 1
                    'ADD DEVD AVRES FROM EXISTING LU PAINT TABLE
                    If Not m_tblExistingLU Is Nothing And Not m_strDWriteValue = "" Then
                        'RETRIEVE VACANT AND DEVELOPABLE ACRES
                        dblDevelopedAcres = 0
                        dblVacantAcres = 0
                        Try
...
CleanUp:
    ...
    Public Sub UpdateEnvisionLyrLegend()
        ...
        If m_pEditFeatureLyr.FeatureClass.FindField("DEV_TYPE") <= -1 Then
            ...
            If Not AddEnvisionField(pTable, "DEV_TYPE", "STRING", 50, 0) Then
                GoTo CleanUp
            End If
        End If
                ...
                For intCount = 0 To intDevTypeCount - 1
                    ...
                    For uh = 0 To (pRender.ValueCount - 1)
                        If blnDevTypFldIsString Then
                            ...
                            End If
                        Else
                            If pRender.Value(uh) = intDevType Then
                                NoValFound = True
                                Exit For
                            End If
                        End If
                    ...
                    If Not ValFound Then
                        If blnDevTypFldIsString Then
                            pRender.AddValue(strDevType, strFieldName, symx)
                            pRender.Label(strDevType) = strDevType
                            pRender.Symbol(strDevType) = symx
                            If strDevType = "" Then
                                pRender.AddReferenceValue(" ", strDevType)
                            End If
                        Else
                            pRender.AddValue(intDevType, strFieldName, symx)
                            pRender.Label(intDevType) = strDevType
                            pRender.Symbol(intDevType) = symx
                        End If
                    End If
                Next
                'GoTo CleanUp
...
CleanUp:
    ...
    Public Function RunTool(ByVal geoProcessor As Geoprocessor, ByVal process As IGPProcess) As Boolean
        '*******************************************************
...
CleanUp:
    ...
    Public Function ReturnMessages(ByVal GP As Geoprocessor) As Boolean
        '*******************************************************
        ...
        Try
            ...
            For intRow = 1 To 10
                ...
                If Not strFldCellValue Is Nothing Then
                    ...
                    Else
                        If strFldCellValue.Contains(",") Then
                            strFieldName = CStr(strFldCellValue.Split(",")(0))
            ...
            For intCol = 1 To 200
                ...
                If strCellValue.Contains(",") Then
                    ...
                    strDepVarFieldName = ""
                    Try
                        strFieldName = CStr(strCellValue.Split(",")(0))
                    Catch ex As Exception
                        strFieldName = ""
                    End Try
                    Try
                        strFieldType = CStr(strCellValue.Split(",")(1))
                    Catch ex As Exception
                        strFieldType = ""
                    End Try
                    Try
                        intFieldWidth = CInt(strCellValue.Split(",")(2))
                    Catch ex As Exception
                        intFieldWidth = 0
                    End Try
                    Try
                        intFieldDecimal = CInt(strCellValue.Split(",")(3))
                    Catch ex As Exception
                        intFieldDecimal = 0
                    End Try
                    Try
                        strAlias = CStr(strCellValue.Split(",")(4))
                    Catch ex As Exception
                        strAlias = ""
                    End Try
                    Try
                        intCalcByAcres = CInt(strCellValue.Split(",")(5))
                    Catch ex As Exception
                        intCalcByAcres = 0
                    End Try
                    Try
                        strCalcFieldName = CStr(strCellValue.Split(",")(6))
                    Catch ex As Exception
                        strCalcFieldName = ""
                    End Try
                    Try
                        strDepVarFieldName = CStr(strCellValue.Split(",")(7))
                    Catch ex As Exception
                        strDepVarFieldName = ""
                    End Try
                ...
                m_dockEnvisionWinForm.prgBarEnvision.Value = (intCol / 200) * 100
            Next
            m_dockEnvisionWinForm.prgBarEnvision.Value = 0
        Catch ex As Exception
...
CleanUp:
    ...
    Public Sub LookUpTablesEnvisionDevTypesTblCheck(ByVal pTable As ITable)
        ...
        Try
            ...
            For intCol = 1 To 250
                ...
                If strCellValue.Contains(",") Then
                    ...
                    End Try
                    Try
                        strFieldCalcName = CStr(strCellValue.Split(",")(4))
                    Catch ex As Exception
                        strFieldCalcName = ""
                    End Try
                    If strFieldName.Length > 0 And intFieldWidth > 0 And strFieldType.Length > 0 Then
                ...
                If pTable.FindField("DEVELOPMENT_TYPE") <= -1 Then
                    If Not AddEnvisionField(pTable, "DEVELOPMENT_TYPE", "STRING", 50, 0) Then
                        GoTo CleanUp
                    End If
                End If
                m_dockEnvisionWinForm.prgBarEnvision.Value = (intCol / 250) * 100
            Next
            m_dockEnvisionWinForm.prgBarEnvision.Value = 0
        Catch ex As Exception
        ...
        If intRowCount < 50 Then
            For intRowCount = intRowCount To 50
                pTable.CreateRow()
            Next
        End If
        intRowCount = pTable.RowCount(Nothing)
        ...
        If intRowCount > 0 Then
            For intRow = 1 To intRowCount
                ...
                For intFld = 1 To intFldCount - 1
                    ...
                    m_appEnvision.StatusBar.Message(0) = "CHECKING FOR REQUIRED DEVELOPMENT TYPE TABLE LOOKUP TABLE FIELDS: CHECKING FOR NULL VALUES; FIELD (" & pField.Name & "); RECORED (" & intRow.ToString & ")"
                    Try
        ...
        Try
            ...
            If strPath.Length > 0 Then
                ...
                If Directory.Exists(strPath) Then
                    ...
                    End If
                    m_strProcessing = m_strProcessing & vbNewLine & "ATTRIBUTE TRACKING TABLE code complete"
                    ...
                    End If
                    m_strProcessing = m_strProcessing & vbNewLine & "DEVELOPMENT TYPES TABLE code complete"
    ...
    Public Sub LoadExcelDevTypes()
        ...
        Try
            If OpenEnvisionExcel() Then
                ...
                m_appEnvision.StatusBar.Message(0) = "Loading Development Types from selected Excel file."
                ...
                If LookUpTablesEnvisionCheck(m_strFeaturePath) Then
                    m_appEnvision.StatusBar.Message(0) = "Retrieveing Development Types"
                    RetrieveDevTypeData()
                    m_dockEnvisionWinForm.itmEnvisionExcel.Enabled = True
                Else
                    GoTo CleanUp
...
CleanUp:
    ...
    Public Sub LookUpTablesEnvisionAttributeFieldTrackingTblCheck(ByVal pTable As ITable)
        ...
        m_strProcessing = m_strProcessing & "Starting function LookUpTablesEnvisionAttributesTrackingTblCheck: " & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
        ...
        If pTable.FindField("FIELD_ALIAS") <= -1 Then
            If Not AddEnvisionField(pTable, "FIELD_ALIAS", "STRING", 50, 0) Then
                GoTo CleanUp
            End If
        End If
        ...
        If pTable.FindField("CALC_FIELD_NAME") <= -1 Then
            If Not AddEnvisionField(pTable, "CALC_FIELD_NAME", "STRING", 50, 0) Then
                GoTo CleanUp
            End If
        End If
        'CHECK FOR CALC FIELD NAME FIELD..Name of the shapefile field is different than dev type field, because is a calculated value
        ...
        If pTable.FindField("DEP_VAR_FIELD_NAME") <= -1 Then
            If Not AddEnvisionField(pTable, "DEP_VAR_FIELD_NAME", "STRING", 50, 0) Then
                GoTo CleanUp
            End If
        End If
        GoTo CleanUp
...
CleanUp:
        m_strProcessing = m_strProcessing & "Ending function LookUpTablesEnvisionFieldTrackingTblCheck: " & Date.Now.ToString("hh:mm:ss tt") & vbNewLine & vbNewLine
        pTable = Nothing
        GC.WaitForPendingFinalizers()
        GC.Collect()
        m_appEnvision.StatusBar.Message(0) = ""
        m_dockEnvisionWinForm.prgBarEnvision.Value = 0
        m_strProcessing = m_strProcessing & vbNewLine & "test end"
    End Sub
...
CleanUp:
    ...
    Public Function LookUpTablesEnvisionScenarioTblCheck(ByVal pTable As ITable) As Boolean
        ...
        For Each strFieldName In strFields.Split(",")
            m_appEnvision.StatusBar.Message(0) = "CHECKING FOR REQUIRED ENVISION SCENRIO TRACKING TABLE FIELDS AND FIELD CALCULATIONS: FIELD (" & strFieldName & ")"
            If pTable.FindField(strFieldName) >= 0 Then
                Continue For
            End If
            If Not AddEnvisionField(pTable, strFieldName, "DOUBLE", 16, 6) Then
            ...
            For intRow = 1 To intRowCount
                ...
                For intFld = 1 To intFldCount - 1
                    ...
                    Else
                        ...
                        End Try
                    End If
                    m_appEnvision.StatusBar.Message(0) = "CHECKING FOR REQUIRED SCENARIO TABLE LOOKUP TABLE FIELDS: CHECKING FOR NULL VALUES; FIELD (" & pField.Name & "); RECORED (" & intRow.ToString & ")"
                Next
                If blnStore Then
                    rowTemp.Store()
                End If
...
CleanUp:
    ...
    Public Function LookUpTablesEnvisionSummaryTblCheck(ByVal pTable As ITable) As Boolean
        ...
        If pTable.FindField("SCENARIO") <= -1 Then
            ...
            If Not AddEnvisionField(pTable, "SCENARIO", "STRING", 50, 0) Then
                Return False
            End If
        End If
        intFld = intFld + 1
...
CleanUp:
    ...
    Public Function LookUpTablesEnvisionEXLUREFTblCheck(ByVal pTable As ITable) As Boolean
        ...
        Dim strEXLU As String = "MF,TH,SF_SM,SF_MD,SF_LRG,MH,RET,OFF,IND,PUB,EDU,HOTEL,UTIL,PKG,AG,OS,VAC,NONE"
        ...
        Try
            'CHECK FOR THE SET NUMBER OF RECORDS
            intRowCount = pTable.RowCount(Nothing)
            If Not intRowCount = 50 Then
                pTable.DeleteSearchedRows(Nothing)
                For intRowCount = 1 To 50
                    rowTemp = pTable.CreateRow()
                    rowTemp.Value(1) = ""
                    rowTemp.Value(2) = ""
                    rowTemp.Value(3) = ""
...
CleanUp:
    ...
    Public Sub LoadDevTypesFromExcel()
        ...
        Try
            ...
            'CYCLE THROUGH EACH DEVELOPMENT TRYPE
            intRowNmbr = 1
            For intRow = intStartRow To (intStartRow + 50)
                ...
                For intCol = 1 To 225
                    ...
                    m_dockEnvisionWinForm.prgBarEnvision.Value = (intProgress / 225) * 100
                Next
                rowDevType.Store()
                rowSc1.Store()
                rowSc2.Store()
    ...
    Public Sub LoadTravelInputsFromExcel()
        ...
        Try
            ...
            If Not TypeOf m_xlPaintWB1.Sheets("Travel Times") Is Microsoft.Office.Interop.Excel.Worksheet Then
                ...
                GoTo CleanUp
            Else
                shtDevType = m_xlPaintWB1.Sheets("Travel Times")
            End If
            ...
            For intRow = intStartRow To (intStartRow + 9)
                ...
                rowTemp.Store()
                intProgress = intProgress + 1
                m_dockEnvisionWinForm.prgBarEnvision.Value = (intProgress / 10) * 100
            Next
            m_dockEnvisionWinForm.prgBarEnvision.Value = 0
    ...
    Public Sub LoadEXLUInputsFromExcel()
        ...
        Try
            ...
            If Not TypeOf m_xlPaintWB1.Sheets("Existing Developed Area") Is Microsoft.Office.Interop.Excel.Worksheet Then
                ...
                GoTo CleanUp
            Else
                shtDevType = m_xlPaintWB1.Sheets("Existing Developed Area")
            ...
            Try
                ...
                shtExistingLandUse = m_xlPaintWB1.Sheets("Existing Land Use")
    ...
    Public Sub RetrieveDevTypeData()
        ...
        Try
            ...
            If m_tblDevelopmentTypes Is Nothing Then
                GoTo CleanUp
            End If
            If m_tblDevelopmentTypes.RowCount(Nothing) < 50 Then
                GoTo CleanUp
            End If
            For intRow = 1 To 50
                m_dockEnvisionWinForm.prgBarEnvision.Value = ((intRow / 50) * 100)
                rowTemp = m_tblDevelopmentTypes.GetRow(intRow)
                Try
            ...
            If intUse = 1 Then
                If intCalcByAcres = 0 Then
                    ...
                    If intCalcByAcres >= 1 Then
                        m_arrWriteDevTypeAcresFields.Add(strFieldName)
                        If intCalcByAcres = 2 Then
                            m_arrWriteDevTypeAcresFields.Add(strDepVarFieldName)
                        End If
                    End If
                End If
            End If
    ...
    Public Sub EnvisionSyncEX_LU_CreateSummaryTablesPaint()
        ...
        Try
            ...
            If strExLUFld.Length > 0 And strDevdAcresPaintedFld.Length > 0 Then
                ...
                m_tblExistingLU = pGeoProc.Dissolve(pLayer, True, strExLUFld, ("Minimum." & strExLUFld & ",Sum." & strDevdAcresPaintedFld), pOutDatasetName)
            End If
        Catch ex As Exception
