...
Public Class frmEnvisionProjectSetup
    ...
    '***************************************************************************************************************************************
    Public blnOpenForm As Boolean = True
    Public pGPSetup As ESRI.ArcGIS.Geoprocessor.Geoprocessor
    Public dtStartTime As Date
    Public strLastAcresFld As String = ""

    'TAB 1 INPUT CHECKS
    Public blnWorkspace As Boolean = False
    Public blnProjectName As Boolean = False
    Public blnSourceInputs As Boolean = True
    Public blnGridCellSize As Boolean = True
    Public blnMaxParcelSize As Boolean = True
    Public blnExtent As Boolean = False
    Public blnProjection As Boolean = False
    'TAB 2 INPUT CHECKS
    Public blnLineBuffer As Boolean = False
    Public blnSlope As Boolean = False
    'TAB 3 
    ...
    Dim m_lyrCentroids As IFeatureLayer = Nothing
...
CleanUp:
    ...
    Private Sub frmEnvisionProjectSetup_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            blnOpenForm = False
            If Not m_pSpatRefProject Is Nothing Then
    ...
    Private Sub btnWorkspace_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnWorkspace.Click
        ...
        If (MyDialog.ShowDialog() = Windows.Forms.DialogResult.OK) Then
            ...
            m_strWorkspacePath = MyDialog.SelectedPath.ToString
        End If
...
CleanUp:
    ...
    Private Sub cmbParcelLayers_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbParcelLayers.SelectedIndexChanged, cmbParcelLayers.TextChanged
        ...
        If Me.cmbParcelLayers.Text.Length <= 0 Or m_blnEnvisonSetupIsOpening Or m_arrPolyFeatureLayers.Count <= 0 Then
            GoTo CleanUp
        Else
        ...
        If m_arrPolyFeatureLayers.Count > 0 And blnLayerFound Then
            Try
                m_lyrConstrainedParcels = CType(m_arrPolyFeatureLayers.Item(Me.cmbParcelLayers.SelectedIndex), IFeatureLayer)
                pFeatureClass = m_lyrConstrainedParcels.FeatureClass
            Catch ex As Exception
...
CleanUp:
    ...
    Private Sub rdbExtentView_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rdbExtentView.CheckedChanged
        ...
        Try
            ...
            Try
                If Not m_pSpatRefProject Is Nothing And Not pSR Is Nothing Then
                    pClone1 = m_pSpatRefProject
                    pClone2 = pSR
                    If Not pClone1.IsEqual(pClone2) And Not pExtentEnv Is Nothing Then
                        pGeometry = pExtentEnv
                        pGeometry.Project(m_pSpatRefProject)
...
CleanUp:
    ...
    Private Sub rdbExtentParcel_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rdbExtentParcel.CheckedChanged, cmbParcelLayers.SelectedIndexChanged
        If m_blnEnvisonSetupIsOpening Or Me.rdbExtentParcel.Checked = False Then
            Exit Sub
        End If
        ...
        Try
            ...
            If Me.cmbParcelLayers.SelectedIndex >= 0 Then
                pLayer = CType(m_arrFeatureLayers.Item(Me.cmbParcelLayers.SelectedIndex), ILayer)
                pGeodataset = pLayer
...
CleanUp:
    ...
    Private Sub cmbLandUseLayers_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbLandUseLayers.TextChanged
        If m_blnEnvisonSetupIsOpening Then
            Exit Sub
        End If
        ...
        If Me.cmbLandUseLayers.Text = "Select the Land Use Feature Layer" Or m_blnEnvisonSetupIsOpening Or m_arrPolyFeatureLayers.Count <= 0 Then
            GoTo CleanUp
        Else
            'MAKE SURE THE SELECTED LAYER IS VALID
...
CleanUp:
    ...
    Private Sub cmbSubaraLayers_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbSubaraLayers.SelectedIndexChanged
        ...
        If m_blnEnvisonSetupIsOpening Or m_arrPolyFeatureLayers.Count <= 0 Then
            Exit Sub
        End If
...
CleanUp:
    ...
    '***************************************************************************************************************************************
    Private Sub btnFormStatus_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'BUILD A STRING LISTING THE STATUS TO ALL THE INPUTS
        Dim strFormStatus As String = ""

        strFormStatus = "TAB 1 INPUTS: PROJECT SETTINGS"
        strFormStatus = strFormStatus & vbNewLine & "Work place directory: " & blnWorkspace.ToString
        strFormStatus = strFormStatus & vbNewLine & "Project Name: " & blnProjectName.ToString
        strFormStatus = strFormStatus & vbNewLine & "Source: " & blnSourceInputs.ToString
        strFormStatus = strFormStatus & vbNewLine & "Grid Cell: " & blnGridCellSize.ToString
        strFormStatus = strFormStatus & vbNewLine & "Maximum Parcel Size: " & blnMaxParcelSize.ToString
        strFormStatus = strFormStatus & vbNewLine & "Extent: " & blnExtent.ToString
        strFormStatus = strFormStatus & vbNewLine & "Excel File: " & Me.tbxExcel.Text
        strFormStatus = strFormStatus & vbNewLine & "Projection: " & blnProjection.ToString
        strFormStatus = strFormStatus & vbNewLine & vbNewLine & "TAB 2 INPUTS: CONSTRAINTS"
        strFormStatus = strFormStatus & vbNewLine & "Slope Inputs: " & blnSlope.ToString
        If Me.dgvLandUseAttributes.SelectedRows.Count > 0 Then
            strFormStatus = strFormStatus & vbNewLine & vbNewLine & "TAB 3 INPUTS: LAND USE"
        End If
        strFormStatus = strFormStatus & vbNewLine & vbNewLine & "TAB 4 INPUTS: SUB-AREAS"
        MessageBox.Show(strFormStatus, "Envision Project Setup Inputs", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly, False)
    End Sub
    ...
    Public Sub CalCulateGridCellCount()
        ...
        If m_pExtentEnv Is Nothing Or m_blnEnvisonSetupIsOpening Then
            GoTo cleanup
        Else
...
CleanUp:
    ...
    Private Sub btnRun_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFinalRun.Click
        ...
        m_appEnvision.StatusBar.Message(0) = "PrRoject Setup: Start"
        ...
        If Me.rdbSourceGridCells.Checked Or Me.rdbSourceHybrid.Checked Then
            ...
            If IsNumeric(Me.rdbSourceGridCells.Checked) Then
                If (CInt(Me.tbxGridCellSize.Text) * CInt(Me.tbxGridCellSize.Text)) > pArea.Area Then
                ...
                End If
            Else
                m_strProcessing = m_strProcessing & vbNewLine & "The grid cell size provided is not a numeric value.  Please review the inputs."
                GoTo ProblemExit
            End If
        End If
        ...
        If Me.rdbSourceGridCells.Checked Or Me.rdbSourceHybrid.Checked Then
            ...
            If CreateGridFeatClass() Then
                If CreateGridCellFeatures() Then
                Else
                    GoTo ProblemExit
                End If
            Else
        ...
        m_strProcessing = m_strProcessing & vbNewLine & vbNewLine & "LAND USE LAYER PROCESSING:" & vbNewLine & Date.Now.ToString("M/d/yyyy hh:mm:ss tt")
        For intRow = 0 To Me.dgvLandUseAttributes.Rows.Count - 1
            ...
            If Me.dgvLandUseAttributes.Rows(intRow).Cells(2).Value = "TRUE" Then
                blnLanduse = True
            End If
            If Me.dgvLandUseAttributes.Rows(intRow).Cells(3).Value = "TRUE" Then
                blnLanduse = True
            End If
        Next
        If blnLanduse Then
            CreateLandUseFeatClass(arrLUfrom, arrLUto)
        End If
...
CleanUp:
    ...
    Private Function CreateAOIFeatClass() As Boolean
        ...
        'CREATE THE AOI FEATURE CLASS, WHICH CONTAIN THE AOI RECTANGLE FEAURE
        ...
        Try
            ...
            pCreateFeatClass.spatial_reference = m_pSpatRefProject
...
CleanUp:
    ...
    Private Function CreateAOIClipOfParcels() As Boolean
        ...
        Try
            ...
            pClip.in_features = m_lyrConstrainedParcels
        ...
        Try
            ...
            If Not m_lyrMaxAcreParcels Is Nothing And Not m_lyrGrid Is Nothing Then
                ...
                If pFeatClass.ShapeType = esriGeometryType.esriGeometryPolygon Then
                    ...
                    pTempLayer.Name = m_lyrParcelsUnionWithGrid.Name & "<SETUP> " & "PROJECT MAXIMUM PARCEL WITH GRID CELLS"
                End If
                If Not m_lyrMinAcreParcels Is Nothing And Not pTempLayer Is Nothing Then
                    ...
                    strInFeatures = pDataset.Workspace.PathName & "\" & pDataset.BrowseName
                    sb.AppendFormat("{0};", strInFeatures)
                    pDataset = CType(m_lyrParcelsUnionWithGrid.FeatureClass, IDataset)
                    strInFeatures = pDataset.Workspace.PathName & "\" & pDataset.BrowseName
                    sb.AppendFormat("{0};", strInFeatures)
            ...
            If pFeatClass.ShapeType = esriGeometryType.esriGeometryPolygon Then
                pTempLayer = New FeatureLayer
                pTempLayer.FeatureClass = pFeatClass
                pTempLayer.Name = m_lyrParcelsUnionWithGrid.Name & "<SETUP> " & "ENVISION PARCELS WITH GRID CELLS"
            End If
        ...
        Try
            ...
            For intCount = 0 To Me.dgvConstraints.Rows.Count - 1
                If Me.dgvConstraints.Rows(intCount).Cells(0).Value = "TRUE" Then
                    pFeatLayer = m_arrFeatureLayers.Item(intCount)
                    Try
...
CleanUp:
    ...
    Private Function CreateLandUseFeatClass(ByVal arrFrom As ArrayList, ByVal arrTo As ArrayList) As Boolean
        'CHECK TO SEE IF THE USER DEFINED LANDUSE INPUTS TO DEVELOP VACANT AND/OR DEVELOPED LANDUSE FEAURE CLASSES
        ...
        'DELETE ALL EXCEPT THE EX_LU FIELD
        Try
            If blnDeleteFields Then
                If Not Me.cmbParcelLayers.SelectedIndex = Me.cmbLandUseLayers.SelectedIndex Then
                    If Not pTable Is Nothing Then
                        If pTable.Fields.FieldCount > 2 Then
                            For intCount = 0 To pTable.Fields.FieldCount - 1
                                If Not pTable.Fields.Field(intCount).Name = "EX_LU" Then
                                    arrFieldNames.Add(pTable.Fields.Field(intCount).Name)
                                End If
                            Next
                        End If
                    End If
                    If arrFieldNames.Count > 0 Then
                        For Each strFieldName In arrFieldNames
                            Try
                                pField = pTable.Fields.Field(pTable.FindField(strFieldName))
                                If Not pField.Type = esriFieldType.esriFieldTypeOID Then
                                    If Not pField Is Nothing Then
                                        pTable.DeleteField(pField)
                                    End If
                                End If
                            Catch ex As Exception
                                m_strProcessing = m_strProcessing & vbNewLine & "Error in deleting excess field(s), " & strFieldName & ", from the clipped land use layer: " & vbNewLine & ex.Message & vbNewLine & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
                            End Try
                        Next
                    End If
                End If
            End If
        Catch ex As Exception
            m_strProcessing = m_strProcessing & vbNewLine & "Error in deleting excess field(s) from the clipped land use layer: " & vbNewLine & ex.Message & vbNewLine & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
        End Try


        'UNION WITH THE CURRENT PARCEL/GRID LAYER
        Try
            If Not m_lyrMainProcessingLayer Is Nothing Then
                ...
                'MAKE SURE THE FIELD EX_LU IS DELETED FROM THE PARCEL LAYER SO THE LAND USE EX_LU FIELD WILL BE USED IN THE OUTPUT LAYER
                If m_lyrMainProcessingLayer.FeatureClass.FindField("EX_LU") >= 0 Then
                    Try
                        pField = m_lyrMainProcessingLayer.FeatureClass.Fields.Field(m_lyrMainProcessingLayer.FeatureClass.FindField("EX_LU"))
                        m_lyrMainProcessingLayer.FeatureClass.DeleteField(pField)
                    Catch ex As Exception
                        m_strProcessing = m_strProcessing & vbNewLine & "EX_LU found in Parcel layer and unable to delete. Error: " & vbNewLine & ex.Message & vbNewLine & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
                    End Try
                End If

                'ADD CONSTRAINTS LAYER TO UNION INPUTS STRING
                Try
                    pDataset = CType(m_lyrMainProcessingLayer.FeatureClass, IDataset)
                    strInFeatures = pDataset.Workspace.PathName & "\" & pDataset.BrowseName
                    sb.AppendFormat("{0};", strInFeatures)
                Catch ex As Exception
                End Try
                'ADD LAND USE LAYER TO UNION INPUTS STRING
                Try
                ...
                pUnion.gaps = "GAPS"
                pUnion.cluster_tolerance = 1
                strUnionName = m_frmEnvisionProjectSetup.tbxWorkspace.Text & "\" & m_frmEnvisionProjectSetup.tbxProjectName.Text & ".gdb" & "\ENVISION_LANDUSE_N_PARCELS_OUTPUT"
                pUnion.out_feature_class = strUnionName
                RunTool(pGPSetup, pUnion)
                pUnion = Nothing
            End If
        Catch ex As Exception
            m_strProcessing = m_strProcessing & vbNewLine & "Error in Unioning Land Use layer to the Envision Parcel layer: " & vbNewLine & ex.Message & vbNewLine & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
            GoTo CleanUp
        End Try
        ...
        Try
            'DISSOLVE THE FINAL CONSTRAINTS LAYER
            pDissolve = New ESRI.ArcGIS.DataManagementTools.Dissolve
            pDissolve.in_features = m_frmEnvisionProjectSetup.tbxWorkspace.Text & "\" & m_frmEnvisionProjectSetup.tbxProjectName.Text & ".gdb" & "\ENVISION_LANDUSE_N_PARCELS_OUTPUT"
            pDissolve.dissolve_field = "ET_TEMP_OBJ_ID2;EX_LU"
            strUnionName = m_frmEnvisionProjectSetup.tbxWorkspace.Text & "\" & m_frmEnvisionProjectSetup.tbxProjectName.Text & ".gdb" & "\ENVISION_LANDUSE_N_PARCELS_OUTPUT_DISSOLVED"

            pDissolve.out_feature_class = strUnionName
            RunTool(pGPSetup, pDissolve)
            pUnion = Nothing
        Catch ex As Exception
            m_strProcessing = m_strProcessing & vbNewLine & "Error in Dissolving Land Use layer to the Envision Parcel layer: " & vbNewLine & ex.Message & vbNewLine & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
            GoTo CleanUp
        End Try

        Try
            'CREATE A SUMMARY TABLE FROM THE DISSOLVED FEATURE CLASS WHERE EACH PARCEL ID HAS THE MAXIMUM VALUE RETURNED ONLY
            pSummarize = New ESRI.ArcGIS.AnalysisTools.Statistics
            pSummarize.case_field = "ET_TEMP_OBJ_ID2"
            pSummarize.statistics_fields = "Shape_Area MAX;EX_LU FIRST"
            pSummarize.in_table = m_frmEnvisionProjectSetup.tbxWorkspace.Text & "\" & m_frmEnvisionProjectSetup.tbxProjectName.Text & ".gdb" & "\ENVISION_LANDUSE_N_PARCELS_OUTPUT_DISSOLVED"
            pSummarize.out_table = m_frmEnvisionProjectSetup.tbxWorkspace.Text & "\" & m_frmEnvisionProjectSetup.tbxProjectName.Text & ".gdb" & "\ENVISION_LANDUSE_N_PARCELS_OUTPUT_DISSOLVED_SUMMARIZED"
            RunTool(pGPSetup, pSummarize)
            pUnion = Nothing
        Catch ex As Exception
            m_strProcessing = m_strProcessing & vbNewLine & "Error in Summary of Dissolved Land Use and Envision Parcel layer output: " & vbNewLine & ex.Message & vbNewLine & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
            GoTo CleanUp
        End Try

        'POPULATE THE LAND USE OUTPUT WITH THE EXISITNG LAND USE ABBREVIATIONS
        Try
            pTable = CType(m_lyrMainProcessingLayer.FeatureClass, Table)
            AddEnvisionField(pTable, "EX_LU", "STRING", 50, 0)
            CalcFldValues(pTable, "EX_LU", """""", "STRING")
            pWksFactory = New FileGDBWorkspaceFactory
            pFeatWks = pWksFactory.OpenFromFile(m_frmEnvisionProjectSetup.tbxWorkspace.Text & "\" & m_frmEnvisionProjectSetup.tbxProjectName.Text & ".gdb", 0)
            pSummaryTable = pFeatWks.OpenTable("ENVISION_LANDUSE_N_PARCELS_OUTPUT_DISSOLVED_SUMMARIZED")
        Catch ex As Exception
            m_strProcessing = m_strProcessing & vbNewLine & "Error in Land Use Summary review: " & vbNewLine & ex.Message & vbNewLine & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
            GoTo CleanUp
        End Try

        Try
            'JOIN THE SUMMARY TABLE TO THE EVISION PARCEL FEATURE CLASS, THEN CALCULATE THE EX_LU field
            pJoin = New ESRI.ArcGIS.DataManagementTools.JoinField
            pJoin.in_data = pTable
            pJoin.in_field = "ET_TEMP_OBJ_ID2"
            pJoin.join_table = pSummaryTable
            pJoin.join_field = "ET_TEMP_OBJ_ID2"
            RunTool(pGPSetup, pJoin)

            If pTable.FindField("EX_LU") >= 0 And pTable.FindField("FIRST_EX_LU") >= 0 Then
                If Not CalcFldValues(pTable, "EX_LU", "[FIRST_EX_LU]", "STRING") Then
                    m_strProcessing = m_strProcessing & vbNewLine & "Error in calculating FIRST_EX_LU to EX_LU: " & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
                    GoTo CleanUp
                End If
            End If
            ...
            pJoin = Nothing
        Catch ex As Exception
            m_strProcessing = m_strProcessing & vbNewLine & "Error in joining land use summary table to parcels and calculate the EX_LU field: " & vbNewLine & ex.Message & vbNewLine & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
        End Try
        GoTo CleanUp
...
CleanUp:
    ...
    Private Sub CreateSetupGP()
        ...
        If pGPSetup Is Nothing Then
            ...
            pGPSetup.SetEnvironmentValue("cellSize", Me.tbxConstraintsCellSize.Text)
...
CleanUp:
    ...
    Private Sub CreateCentroidLayer()
        'CREATE A CENTROID POINT FEATURE LAYER FROM THE DEFINED PARCEL LAYER
        m_strProcessing = m_strProcessing & vbNewLine & "Starting function CreateCentroidLayer: " & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
        Dim pWksFactory As IWorkspaceFactory = Nothing
        Dim pFeatWks As IFeatureWorkspace
        ...
        If Not m_lyrEnvisionParcel Is Nothing Then
            ...
            Try
                ...
                CreateCentroidFeatClass()
                CreateCentroidPointFeatures(pToFeatClass)

                GoTo CleanUp

            Catch ex As Exception
                m_strProcessing = m_strProcessing & vbNewLine & "Error in function CreateCentroidLayer.  Failed on feature count:  " & intTotalCount.ToString & vbNewLine & "Error: " & ex.Message
                GoTo CleanUp
            End Try
        End If
...
CleanUp:
        ...
        End Try
        m_strProcessing = m_strProcessing & vbNewLine & "End function CreateCentroidLayer: " & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
        Me.lblRunStatus.Text = ""
        pWksFactory = Nothing
        pFeatWks = Nothing
    ...
    Private Function CreateCentroidFeatClass() As Boolean
        CreateCentroidFeatClass = True
        m_strProcessing = m_strProcessing & vbNewLine & "Starting function CreateCentroidFeatClass: " & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
        ...
        Try
            ...
            pCreateFeatClass = New ESRI.ArcGIS.DataManagementTools.CreateFeatureclass
            pCreateFeatClass.out_path = m_frmEnvisionProjectSetup.tbxWorkspace.Text & "\" & m_frmEnvisionProjectSetup.tbxProjectName.Text & ".gdb"
            pCreateFeatClass.out_name = "PROJECT_CENTROIDS"
            ...
            pWksFactory = New FileGDBWorkspaceFactory
            pFeatWks = pWksFactory.OpenFromFile(m_frmEnvisionProjectSetup.tbxWorkspace.Text & "\" & m_frmEnvisionProjectSetup.tbxProjectName.Text & ".gdb", 0)
            pFeatClass = pFeatWks.OpenFeatureClass("PROJECT_CENTROIDS")
            pTable = CType(pFeatClass, ITable)
            'ADD FIELD FOR ENVISION IDS
            AddEnvisionField(pTable, "ET_TEMP_OBJ_ID2", "INTEGER", 16, 0)
            If pFeatClass.ShapeType = esriGeometryType.esriGeometryPoint Then
                m_lyrCentroids = New FeatureLayer
                m_lyrCentroids.FeatureClass = pFeatClass
                m_lyrCentroids.Name = m_lyrGrid.Name & "<SETUP> " & "CENTROID LAYER"
            End If
        Catch ex As Exception
            m_strProcessing = m_strProcessing & vbNewLine & "Error in creating centroid feature class (PROJECT_CENTROIDS): " & ex.Message
            CreateCentroidFeatClass = False
            GoTo CleanUp
        End Try
CleanUp:
        m_strProcessing = m_strProcessing & vbNewLine & "End function CreateCentroidFeatClass: " & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
        pCreateFeatClass = Nothing
        pWksFactory = Nothing
        pFeatWks = Nothing
        pFeatClass = Nothing
    ...
    Private Function CreateCentroidPointFeatures(ByVal pParcelFeatClass As IFeatureClass) As Boolean
        'GENERATE THE GRID CELL POLYGONS
        m_strProcessing = m_strProcessing & vbNewLine & "Starting function CreateCentroidPointFeatures: " & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
        CreateCentroidPointFeatures = True
        ...
        Dim intIdFromField As Integer
        Dim intIdToField As Integer

        Try
            'EXIT IF THERE IS NOT CENTROID FEATURECLASS
            If m_lyrCentroids Is Nothing Then
                CreateCentroidPointFeatures = False
                GoTo CleanUp
            Else
                pCentroidFeatClass = m_lyrCentroids.FeatureClass
            End If

            'RETRIEVE THE FROM AND TO ENVISION ID 2 FIELD NUMBERS
            intIdFromField = pParcelFeatClass.FindField("ET_TEMP_OBJ_ID2")
            intIdToField = pCentroidFeatClass.FindField("ET_TEMP_OBJ_ID2")
            If intIdFromField <= -1 Or intIdToField <= -1 Then
            ...
            Do While Not pFeat Is Nothing
                Try
                    ...
                    If Not pArea Is Nothing Then
                        ...
                        pInsertFeatureBuffer.Value(intIdToField) = intId
                        pInsertFeatureCursor.InsertFeature(pInsertFeatureBuffer)
                        pPointTemp = Nothing
                        pArea = Nothing
                    End If
...
CleanUp:
        m_strProcessing = m_strProcessing & vbNewLine & "End function CreateCentroidPointFeatures: " & Date.Now.ToString("hh:mm:ss tt") & vbNewLine
