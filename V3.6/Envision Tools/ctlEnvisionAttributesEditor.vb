...
Public Class ctlEnvisionAttributesEditor
  ...
  Private Sub itmOpenEnvisionFGDB_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmOpenEnvisionFGDB.Click
    ...
    Try
      ...
      pFGDBFilterColl = DirectCast(CreateObject("esriCatalogUI.GxObjectFiltercollection"), IGxObjectFilterCollection)
      pLyrFilter = New GxFilterFileGeodatabases
      pFGDBFilterColl.AddFilter(pLyrFilter, True)

      With pGxDialog
        .ObjectFilter = DirectCast(pFGDBFilterColl, IGxObjectFilter)
        ...
        Else
          Try
            ...
            'CHECK THE CURRENT DIRECTORY TO ENSURE THERE ISN'T A LOCK on the 
    ...
    If Not m_tblScSummary Is Nothing Then
      Try
        For intCount = 1 To 4
          ...
          strLayerName = rowTemp.Value(m_tblScSummary.FindField("LAYER_NAME")).ToString
      ...
      Do While blnLyrSelected = False
        ...
        Else
          ...
          For intCount = 1 To 4
            ...
            If rowName.Value(m_tblScSummary.FindField("LAYER_NAME")).ToString = strLayerName Then
              blnFound = True
              Exit For
            End If
          Next
...
CleanUp:
  ...
  Private Sub SynchronizeData(ByVal strSelection As String)
    ...
    pDef = DirectCast(m_pEditFeatureLyr, IFeatureLayerDefinition2)
    ...
    Try
      ...
      Do While Not pFeat Is Nothing
        ...
        Catch ex As Exception
          dblDevelopedAcres = 0
        End Try
        dblVacantAcres = Round(dblVacantAcres, 6)
        ...
        If m_blnRunCalcs Then
          ...
          If intDevTypeRow = -1 Then
            ...
            End If
            Dim n As Int32 = 0
            For Each fldName As String In m_arrWriteDevTypeTotalFields
              existingFldIndex = pFeatClass.FindField("EX_" & fldName)
              If existingFldIndex <> -1 Then
                ...
                End If
                totFldIndex = pFeatClass.FindField(m_arrWriteDevTypeTotalFieldName(n).ToString)
            ...
            If m_arrWriteDevTypeFields.Count > 0 Then
              For intFldCount = 0 To m_arrWriteDevTypeFields.Count - 1
                ...
                If m_arrWriteDevTypeTotalFields.IndexOf(strFromFieldName) <> -1 Then
                  existingFldIndex = pFeatClass.FindField("EX_" & strFromFieldName)
                  If existingFldIndex <> -1 Then
                    Dim existingVal As Double = 0
                    totFldIndex = pFeatClass.FindField(m_arrWriteDevTypeTotalFieldName(intFldCount).ToString)
                    If pFeat.Value(existingFldIndex) IsNot DBNull.Value Then
                      existingVal = CType(pFeat.Value(existingFldIndex), Double)
                    End If
                    If totFldIndex <> -1 Then pFeat.Value(totFldIndex) = (CType(rowDevType.Value(m_tblDevelopmentTypes.FindField(strFromFieldName)), Double) * dblRedevRate) + (existingVal * (1 - dblRedevRate))
                  End If
                End If
              Next
            End If
                ...
                Try
                  If intRedevFld <= -1 Then
                    ...
                    If m_arrWriteDevTypeTotalFields.IndexOf(strToFieldName) <> -1 Then
                      totFldIndex = pFeatClass.FindField(m_arrWriteDevTypeTotalFieldName(intFldCount).ToString)
                      If totFldIndex <> -1 Then pFeat.Value(totFldIndex) = 0
                    End If
                  Else
                    ...
                    If m_arrWriteDevTypeTotalFields.IndexOf(strToFieldName) <> -1 Then
                      ...
                      If existingFldIndex <> -1 Then
                        ...
                        If totFldIndex <> -1 Then pFeat.Value(totFldIndex) = dblCalcAcres + (existingVal * (1 - dblRedevRate))
                      End If
                    End If
                    ...
                    If dblExistingXValue > 0 Then
                      ...
                      If intScExField > -1 Then
                        Try
                          dblScExValue = CType(rowScLookUp.Value(intScExField), Double)
                          dblScExValue = Round(dblScExValue, 6)
                        Catch ex As Exception
                          dblScExValue = 0
                        End Try
  ...
  Public Sub EnvisionSyncEX_LU_CreateSummaryTables()
    ...
    Try
      ...
      If strExLUFld.Length > 0 And strDevdAcresPaintedFld.Length > 0 Then
        ...
        pSumTable = pGeoProc.Dissolve(DirectCast(pLayer, ITable), True, strExLUFld, ("Minimum." & strExLUFld & ",Sum." & strDevdAcresPaintedFld), pOutDatasetName)
      End If
    Catch ex As Exception
    ...
    Try
      ...
      If strExLUFld.Length > 0 And strDevdAcresAllFld.Length > 0 Then
        ...
        m_tblExistingLU = pGeoProc.Dissolve(DirectCast(pLayer, ITable), False, strExLUFld, ("Minimum." & strExLUFld & ",Sum." & strDevdAcresAllFld), pOutDatasetName)
      End If
    Catch ex As Exception
    ...
    Try
      If Not m_tblExistingLU Is Nothing Then
        ...
        For intRow = 1 To pSumTable.RowCount(Nothing)
          ...
          strFldCellValue = rowPainted.Value(1).ToString
          For intTblRow = 1 To m_tblExistingLU.RowCount(Nothing)
            rowAll = m_tblExistingLU.GetRow(intTblRow)
            If strFldCellValue = rowAll.Value(1).ToString Then
...
CleanUp:
  ...
  Public Sub EnvisionSyncEX_LU()
    ...
    Try
      ...
      'RETRIEVE THE EXISTING CONDITIONS TAB
      Try
        shtExisting = DirectCast(m_xlPaintWB1.Sheets("Existing Land Use"), Microsoft.Office.Interop.Excel.Worksheet)
    ...
    Dim lastCol As Int32
    intCol = 1
    Dim blankColCt As Int32
    Do While blankColCt < 10
      strFldCellValue = CStr(CType(shtExisting.Cells(intFieldRow, intCol), Microsoft.Office.Interop.Excel.Range).Value)
      If Not strFldCellValue Is Nothing Then
        lastCol = intCol
        blankColCt = 0
    ...
    'CYCLE THROUGH THE COULMNS FOR FIELDS TO RETURN STATISTCS FOR
    intCount = 0
    m_dockEnvisionWinForm.prgBarEnvision.Value = CType((intCount / lastCol) * 100, Int32)
    m_dockEnvisionWinForm.prgBarEnvision.Refresh()
    For intCol = 1 To lastCol
      intCount = intCount + 1
      strFldCellValue = CStr(CType(shtExisting.Cells(intFieldRow, intCol), Microsoft.Office.Interop.Excel.Range).Value)
  ...
  Private Sub itmColorSelector_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmColorSelector.Click
    ...
    Try
      ...
      If bColorSet Then
        ...
        pNewColor.RGB = pSelector.Color.RGB
...
CleanUp:
  ...
  Private Sub itmColorBrowser_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmColorBrowser.Click
    ...
    Try
      ...
      If bColorSet Then
        pNewColor = New RgbColor
        pNewColor.RGB = pColorBrowser.Color.RGB
...
CleanUp:
  ...
  Private Sub itmScenario_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmScenario1.Click, itmScenario2.Click, itmScenario3.Click, itmScenario4.Click, itmScenario5.Click
    ...
    Dim itmSelected As ToolStripMenuItem = DirectCast(sender, Windows.Forms.ToolStripMenuItem)
    UncheckAllScenarios()
    ...
    If ChangeEnvisionLayer() Then
      Dim blnSynchronize As Boolean = True
      'APPLY THE SUBAREA IF ONE IS DEFINED
      Try
        If m_dockEnvisionWinForm.itmcmbSubareaFields.Tag.ToString.Length > 0 And m_dockEnvisionWinForm.itmSelectSubarea.Tag.ToString.Length > 0 Then
          If m_dockEnvisionWinForm.itmcmbSubareaFields.FindString(m_dockEnvisionWinForm.itmcmbSubareaFields.Tag.ToString) <> -1 Then
            m_dockEnvisionWinForm.itmcmbSubareaFields.Text = m_dockEnvisionWinForm.itmcmbSubareaFields.Tag.ToString
            If m_dockEnvisionWinForm.itmSelectSubarea.FindString(m_dockEnvisionWinForm.itmSelectSubarea.Tag.ToString) <> -1 Then
              blnSynchronize = False
              m_dockEnvisionWinForm.itmSelectSubarea.Text = m_dockEnvisionWinForm.itmSelectSubarea.Tag.ToString
            End If
          End If
        End If
      Catch ex As Exception
...
CleanUp:
  ...
  Private Sub itmSaveToExcelFile_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmSaveToExcelFile.Click
    ...
    Dim msg As String = "Please select the Envision Excel file before saving."
    If m_xlPaintWB1 Is Nothing Or m_xlPaintApp Is Nothing Then
      MessageBox.Show(msg, "Select Envision Excel File")
      Exit Sub
    Else
      Try
        ' This will cause an exception if the Excel file has been manually closed by the user
        Dim rev As Int32 = m_xlPaintWB1.RevisionNumber
      Catch ex As Exception
        MessageBox.Show(msg, "Select Envision Excel File")
        Exit Sub
      End Try
...
CLeanUp:
  ...
  Private Sub itmSelectSubarea_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmSelectSubarea.SelectedIndexChanged, itmSelectSubarea.Enter
    ...
    Try
      ...
      pFLDef = DirectCast(m_pEditFeatureLyr, IFeatureLayerDefinition)
...
CleanUp:
  ...
  Private Sub ctlEnvisionAttributesEditor_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    ' Set the ET version and build numbers
    Dim version As System.Version = Assembly.GetExecutingAssembly().GetName().Version
    Dim dateTimeOfBuild As DateTime = New DateTime(2000, 1, 1) + New TimeSpan(version.Build, 0, 0, 0)
    itemETVersion.Text = "Envision Tomorrow " & version.Major.ToString & "." & version.Minor.ToString & "." & version.Build.ToString & " (" & dateTimeOfBuild.ToString("M/d/yy") & ")"

    'SET THE CALCULATION VARIABLE - ON
    If m_blnRunCalcs Then
...
CleanUp:
  ...
  Private Sub LoadPolyLayers()
    ...
    Else
      Try
        pWSF = New FileGDBWorkspaceFactory
        pFeatureWS = DirectCast(pWSF.OpenFromFile(m_strFeaturePath, 0), IFeatureWorkspace)
        'Insert Clear Option
...
CleanUp:
  ...
  Private Sub cmbDefineLayer1_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbDefineLayer1.SelectedIndexChanged, cmbDefineLayer2.SelectedIndexChanged, cmbDefineLayer3.SelectedIndexChanged, cmbDefineLayer4.SelectedIndexChanged, cmbDefineLayer5.SelectedIndexChanged
    ...
    For intCount = 1 To 5
      ...
      strLayerName = rowScTemp.Value(m_tblScSummary.FindField("LAYER_NAME")).ToString
  ...
  Public Function QueryNulls(ByVal pFeatClass As IFeatureClass, ByVal strFieldName As String) As Boolean
    ...
    Try
      ...
      If pFeatSelection.SelectionSet.Count > 0 Then
        ...
        pCursor = DirectCast(pFeatClass.Update(pQFilter, False), ICursor)
        With pCalc
          .Cursor = pCursor
          .Field = strFieldName
          .Expression = "0"
...
CleanUp:
  ...
  Private Sub itmFieldManager_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmFieldManager.Click
    ...
    ' Make ArcMap the owner of this form.
    OnTop(m_frmAttribManager.Handle.ToInt32, m_appEnvision.hWnd)
  ...
  Private Sub itmCalcsOn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmCalcsOn.Click
    ...
    ' If the Excel is not open prompt the user to select it
    If m_xlPaintWB1 Is Nothing OrElse m_xlPaintApp Is Nothing Then SelectEnvisionExcelFile()
  End Sub
  ...
  Private Sub itmJHBalance_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmJHBalance.Click
    ...
    ' Make ArcMap the owner of this form.
    OnTop(m_frmLocalJHBalance.Handle.ToInt32, m_appEnvision.hWnd)
  ...
  Private Sub itmRedevTiming_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmRedevTiming.Click
    ...
    ' Make ArcMap the owner of this form.
    OnTop(m_frmRedevTiming.Handle.ToInt32, m_appEnvision.hWnd)

    m_frmRedevTiming.Show()
  End Sub

  Private Sub itmRedevFeasibility_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmRedevFeasibility.Click
    'OPENS THE REDEVELOPMENT FEASIBILTY FORM
    If m_frmDevFeasibility Is Nothing Then
      m_frmDevFeasibility = New frmDevFeasibility
    End If

    ' Make ArcMap the owner of this form.
    OnTop(m_frmDevFeasibility.Handle.ToInt32, m_appEnvision.hWnd)

    m_frmDevFeasibility.Show()
  End Sub

  Private Sub itmStationLocation_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles itmStationLocation.Click
    'OPENS THE LOCATION EFFICIENCY TOOL FORM
    If m_frmLocationEfficiency Is Nothing Then
      m_frmLocationEfficiency = New frmLocationEfficiency
    End If

    ' Make ArcMap the owner of this form.
    OnTop(m_frmLocationEfficiency.Handle.ToInt32, m_appEnvision.hWnd)

    m_frmLocationEfficiency.Show()
  End Sub

  Private Sub itemEPATData_Click(sender As Object, e As EventArgs) Handles itemEPAPData.Click
    ' Open the form
    Dim frmHIA As New frmHIA
    frmHIA.ShowDialog()
  End Sub
  ...
  Private Sub TestToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Try
      If TypeOf m_frmLocationEfficiency Is IApplication Then
        pMxDocument = DirectCast(m_appEnvision.Document, IMxDocument)
        mapCurrent = CType(pMxDocument.FocusMap, Map)
        pRLayer = DirectCast(mapCurrent.Layer(0), IRasterLayer)
        pRaster = pRLayer.Raster
        pClassRen = New RasterClassifyColorRampRendererClass()
        pRasRen = DirectCast(pClassRen, IRasterRenderer)
    ...
    While j <= range
      ...
      j = j + incrementSize
    End While
    Dim pFreq As Integer() = New Integer(size - 1) {}
    For k As Integer = 0 To size - 1
      pFreq(k) = Convert.ToInt32(temp(k))
    Next
    Dim sd As StandardDeviation = New StandardDeviationClass()
    Dim pClassifyGen As IClassifyGEN = TryCast(sd, IClassifyGEN)
    Dim pDeviationInterval As IDeviationInterval = TryCast(sd, IDeviationInterval)
    ...
    Catch ex As Exception
      MessageBox.Show(ex.Message)
    End Try
    Dim classbreak As Double() = TryCast(pClassifyGen.ClassBreaks, [Double]())
    'FAILS HERE FOR GRID - null
    Dim pUID As UID = pClassifyGen.ClassID
    ...
    pRasRen.Update()
    pRLayer.Renderer = DirectCast(pClassRen, IRasterRenderer)
  ...
  Public Function StretchRenderer(ByVal pRaster As IRaster) As IRasterRenderer
    Try
      'Define the from and to colors for the color ramp.
      Dim fromColor As IRgbColor = New RgbColorClass()
      fromColor.Red = 190
      ...
      Dim pClassifyRenderer As IRasterClassifyColorRampRenderer = New RasterClassifyColorRampRendererClass()
      Dim pStretchRenderer As IRasterStretchColorRampRenderer = New RasterStretchColorRampRenderer
      Dim pRasterRenderer As IRasterRenderer = CType(pStretchRenderer, IRasterRenderer)

      'Set the renderer properties.
      pRasterRenderer.Raster = pRaster
      pRasterRenderer.Update()
      pStretchRenderer.BandIndex = 0
      pStretchRenderer.ColorRamp = colorRamp
      ...
      stretchType.StandardDeviationsParam = 0.5
