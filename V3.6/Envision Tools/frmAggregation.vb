...
Public Class frmAggregation
  ...
  Dim totalSumFldList As List(Of String) = New List(Of String)
  Dim weightedSumFldList As List(Of String) = New List(Of String)
  Dim weightedSumOnlyList As List(Of Boolean) = New List(Of Boolean)
  Dim totalSumOnlyList As List(Of Boolean) = New List(Of Boolean)
  ...
  Dim gpAggregation As ESRI.ArcGIS.Geoprocessor.Geoprocessor = New ESRI.ArcGIS.Geoprocessor.Geoprocessor
  ...
  Private Sub frmAggregation_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    For i As Int32 = 0 To Me.dgvFields.RowCount - 1
      Dim attributeFieldName As String = Me.dgvFields.Rows(i).Cells(1).Value
      Dim aliasName As String = Me.dgvFields.Rows(i).Cells(2).Value
      Dim totalFieldName As String = CStr(Me.dgvFields.Rows(i).Cells(5).Value)
      Dim cursor As ICursor = m_tblAttribFields.Search(Nothing, False)
      Dim row As IRow = cursor.NextRow
      Do While row IsNot Nothing
        Dim calcByAcres As Boolean = False
        If aliasName.Contains("(Calculate Sum)") OrElse aliasName.Contains("(Dependent Var Sum)") Then calcByAcres = True
        Dim storedFieldName As String
        If calcByAcres Then
          storedFieldName = CStr(row.Value(row.Fields.FindField("Calc_Field_Name")))
        Else
          storedFieldName = CStr(row.Value(row.Fields.FindField("Field_Name")))
        End If
        If storedFieldName = attributeFieldName Then
          If calcByAcres Then
            row.Value(row.Fields.FindField("Tot_Fld_Name_Sum_By_Acres")) = totalFieldName
          Else
            row.Value(row.Fields.FindField("Tot_Fld_Name_Sum")) = totalFieldName
          End If
          row.Store()
          Exit Do
        End If
        row = cursor.NextRow
      Loop
      Me.barStatus.Value = (i / Me.dgvFields.RowCount) * 100
      Me.Refresh()
  ...
  Private Sub frmAggregation_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Dim totFieldName As String = String.Empty
    Dim totFieldNameByAcres As String = String.Empty
    ...
    Do Until rowTemp Is Nothing
      ...
      intCalcAcresOnly = 0
      strCalcFieldName = ""
      totFieldName = String.Empty
      totFieldNameByAcres = String.Empty
      Try
      ...
      Catch ex As Exception
      End Try
      If Not IsDBNull(rowTemp.Value(rowTemp.Fields.FindField("Tot_Fld_Name_Sum"))) Then totFieldName = rowTemp.Value(rowTemp.Fields.FindField("Tot_Fld_Name_Sum")).ToString
      If totFieldName = String.Empty Then
        totFieldName = "TOT_" & strFieldName
      End If
      If Not IsDBNull(rowTemp.Value(rowTemp.Fields.FindField("Tot_Fld_Name_Sum_By_Acres"))) Then totFieldNameByAcres = rowTemp.Value(rowTemp.Fields.FindField("Tot_Fld_Name_Sum_By_Acres")).ToString
      If totFieldNameByAcres = String.Empty Then
        totFieldNameByAcres = "TOT_" & strCalcFieldName
      End If
        ...
        If strFieldName.Length > 0 Then
          If Not UCase(strFieldName) = "DEV_TYPE" And Not UCase(strFieldName) = "VAC_ACRES" And Not UCase(strFieldName) = "DEVD_ACRES" _
            ...
            If m_pEditFeatureLyr.FeatureClass.FindField(strFieldName) >= 0 Then
              ...
              Me.dgvFields.Rows(Me.dgvFields.RowCount - 1).Cells(5).Value = totFieldName
            End If
            ...
            If intCalcAcres = 1 Then
              If m_pEditFeatureLyr.FeatureClass.FindField(strCalcFieldName) >= 0 Then
                ...
                Me.dgvFields.Rows(Me.dgvFields.RowCount - 1).Cells(5).Value = totFieldNameByAcres
              End If
            End If
            'CHECK FOR THE SUM VARIABLES
...
CleanUp:
  ...
  Private Sub btnRun_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRun.Click
    ...
    ' Make sure all edits that are in process are committed
    If m_frmFieldAggregation.dgvFields.IsCurrentCellDirty Then
      m_frmFieldAggregation.dgvFields.CommitEdit(DataGridViewDataErrorContexts.Commit)
    End If
    ...
    If m_pEditFeatureLyr.FeatureClass.Fields.FindField("VAC_ACRE") >= 0 Then
      ...
      End If
      If pFeatureClass.FindField("TOT_VAC_SQ_MI") = -1 Then
        AddEnvisionField(pToTable, "TOT_VAC_SQ_MI", "DOUBLE", 16, 6)
      End If
    End If
    ...
    If m_pEditFeatureLyr.FeatureClass.Fields.FindField("VAC_ACRE") >= 0 And m_pEditFeatureLyr.FeatureClass.Fields.FindField("DEVD_ACRE") >= 0 Then
      If pFeatureClass.FindField("TOT_SQ_MI") = -1 Then
        AddEnvisionField(pToTable, "TOT_SQ_MI", "DOUBLE", 16, 6)
      End If
    End If
    ...
    Try
      For intRow = 0 To Me.dgvFields.RowCount - 1
        If m_frmFieldAggregation.dgvFields.Rows(intRow).Cells(0).Value = 1 Then
          ...
          totFldName = Me.dgvFields.Rows(intRow).Cells(5).Value
          Dim totalOnly As Boolean = Me.dgvFields.Rows(intRow).Cells(6).Value = 1
          If m_frmFieldAggregation.dgvFields.Rows(intRow).Cells(3).Value = 1 Then
            arrWeightedSum.Add(strFieldName)
            weightedSumFldList.Add(totFldName)
            weightedSumOnlyList.Add(totalOnly)
          Else
            arrTotalSum.Add(strFieldName)
            totalSumFldList.Add(totFldName)
            totalSumOnlyList.Add(totalOnly)
          End If
          If Not totalOnly Then
            ...
            End If
          End If
          If pFeatureClass.FindField(totFldName) = -1 Then
            AddEnvisionField(pToTable, totFldName, "DOUBLE", 16, 6)
          End If
        End If
      Next
    Catch ex As Exception
    ...
    'ONLY RUN IF MORE THAN 1 FEATURE IS FOUND
...
CleanUp:
  ...
  Private Sub SelectedNeighborhoodFeature(ByVal strQString As String)
    ...
    Try
      ...
      pFeatWrite.Value(pFeatWrite.Fields.FindField("TOT_VAC_SQ_MI")) = (dblTOT_VAC_ACRE * 0.0015625)
      ...
      pFeatWrite.Value(pFeatWrite.Fields.FindField("TOT_SQ_MI")) = (dblTOT_DEVD_ACRE * 0.0015625) + (dblTOT_VAC_ACRE * 0.0015625)
      pFeatWrite.Store()
    Catch ex As Exception
    ...
    Try
      If arrWeightedSum.Count > 0 Then
        For intNum = 1 To arrWeightedSum.Count
          If Not weightedSumOnlyList.Item(intNum - 1) Then
            ...
            End If
          End If
          If pFeatWrite.Fields.FindField(weightedSumFldList(intNum - 1)) >= 0 Then
            ...
            If CDbl(arrTempDomWeightedTotSum.Item(intNum - 1)) = 0 Then
              pFeatWrite.Value(pFeatWrite.Fields.FindField(weightedSumFldList(intNum - 1))) = 0
            Else
              pFeatWrite.Value(pFeatWrite.Fields.FindField(weightedSumFldList(intNum - 1))) = dblTemp
            End If
          End If
        Next
      End If
      ...
      If arrTotalSum.Count > 0 Then
        For intNum = 1 To arrTotalSum.Count
          If Not totalSumOnlyList.Item(intNum - 1) Then
            strFieldName = arrTotalSum.Item(intNum - 1)
            If pFeatWrite.Fields.FindField("EX_PRE_" & strFieldName) >= 0 Then
            ...
            End If
          End If
          If pFeatWrite.Fields.FindField(totalSumFldList(intNum - 1)) >= 0 Then
            pFeatWrite.Value(pFeatWrite.Fields.FindField(totalSumFldList(intNum - 1))) = CDbl(arrTempTotalSum.Item(intNum - 1))
          End If
        Next
      End If
