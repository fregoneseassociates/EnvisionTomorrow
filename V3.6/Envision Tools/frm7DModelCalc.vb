...
CleanUp:
    ...
    Private Sub btnRun_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRun.Click
        ...
        Dim dblHH As Double = 0
        Dim strReport As String = ""
        Dim datWriteLC As StreamWriter
        Dim dblVMTSacleFactor As Double = 1.505
        ...
        If Me.rdbExisting.Checked Then
            intHUFld = pFeatureClass.FindField("EX_PRE_HH")
        Else
            intHUFld = pFeatureClass.FindField("TOT_HH")
        End If
            ...
            Else
                ...
                If arrFldValues.Count > 3 Then
                    If arrFldValues.Item(0) > 0 Then 'And arrFldValues.Item(1) <= 0 And arrFldValues.Item(2) <= 0 Then
                        ...
                        strReport = strReport & "Starting VMT Value:" & dblVMT.ToString & vbNewLine
                        ...
                        For intCount = 0 To arrFldValues.Count - 1
                            ...
                            strReport = strReport & "Input Value (" & intCount.ToString & "):  " & dblInputValue.ToString & vbNewLine

                            If dblInputValue < 0 Then Continue For
                            ...
                            Try
                                ...
                                strReport = strReport & vbNewLine & "START VMT prob (" & intCount.ToString & ")" & strValue.ToString & vbNewLine

                                If strValue.Contains("*") Or strValue.Contains("+") Or strValue.Contains("!") Then
                                    If strValue.Contains("*") Then
                                        strValue = strValue.Replace("*", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "VMT prob Array Value *(" & intCount.ToString & ")" & (dblCoeff * Math.Log(dblInputValue)).ToString & vbNewLine
                                        dblProbVMT = dblProbVMT + (dblCoeff * Math.Log(dblInputValue))
                                    ElseIf strValue.Contains("+") Then
                                        strValue = strValue.Replace("+", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "VMT prob Array Value +(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue / 1000)).ToString & vbNewLine
                                        dblProbVMT = dblProbVMT + (dblCoeff * (dblInputValue / 1000))
                                    ElseIf strValue.Contains("!") Then
                                        strValue = strValue.Replace("!", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "VMT prob Array Value !(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue * 100)).ToString & vbNewLine
                                        dblProbVMT = dblProbVMT + (dblCoeff * (dblInputValue * 100))
                                    End If
                                Else
                                    dblCoeff = CDbl(strValue)
                                    strReport = strReport & "VMT prob Array Value none(" & intCount.ToString & ")" & ((dblCoeff * dblInputValue)).ToString & vbNewLine
                                    dblProbVMT = dblProbVMT + (dblCoeff * dblInputValue)
                                End If

                                strReport = strReport & "NEW VMT prob (" & intCount.ToString & ")" & dblProbVMT.ToString & vbNewLine & vbNewLine
                            ...
                            Try
                                ...
                                strReport = strReport & "VMT Array Value (" & intCount.ToString & ")" & strValue.ToString & vbNewLine
                                If strValue.Contains("*") Or strValue.Contains("+") Or strValue.Contains("!") Then
                                    If strValue.Contains("*") Then
                                        strValue = strValue.Replace("*", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "VMT Array Value *(" & intCount.ToString & ")" & (dblCoeff * Math.Log(dblInputValue)).ToString & vbNewLine
                                        dblVMT = dblVMT + (dblCoeff * Math.Log(dblInputValue))
                                    ElseIf strValue.Contains("+") Then
                                        strValue = strValue.Replace("+", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "VMT Array Value +(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue / 1000)).ToString & vbNewLine
                                        dblVMT = dblVMT + (dblCoeff * (dblInputValue / 1000))
                                    ElseIf strValue.Contains("!") Then
                                        strValue = strValue.Replace("!", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "VMT Array Value !(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue * 100)).ToString & vbNewLine
                                        dblVMT = dblVMT + (dblCoeff * (dblInputValue * 100))
                                    End If
                                Else
                                    dblCoeff = CDbl(strValue)
                                    dblVMT = dblVMT + (dblCoeff * dblInputValue)
                                End If
                                strReport = strReport & "NEW VMT  (" & intCount.ToString & ")" & dblVMT.ToString & vbNewLine & vbNewLine
                            ...
                            Try
                                ...
                                strReport = strReport & vbNewLine & "START Vehicle (" & intCount.ToString & ")" & strValue.ToString & vbNewLine
                                If strValue.Contains("*") Or strValue.Contains("+") Or strValue.Contains("!") Then
                                    If strValue.Contains("*") Then
                                        strValue = strValue.Replace("*", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Vehicle Array Value *(" & intCount.ToString & ")" & (dblCoeff * Math.Log(dblInputValue)).ToString & vbNewLine
                                        dblVehicle = dblVehicle + (dblCoeff * Math.Log(dblInputValue))
                                    ElseIf strValue.Contains("+") Then
                                        strValue = strValue.Replace("+", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Vehicle Array Value +(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue / 1000)).ToString & vbNewLine
                                        dblVehicle = dblVehicle + (dblCoeff * (dblInputValue / 1000))
                                    ElseIf strValue.Contains("!") Then
                                        strValue = strValue.Replace("!", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Vehicle Array Value !(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue * 100)).ToString & vbNewLine
                                        dblVehicle = dblVehicle + (dblCoeff * (dblInputValue * 100))
                                    End If
                                Else
                                    dblCoeff = CDbl(strValue)
                                    dblVehicle = dblVehicle + (dblCoeff * dblInputValue)
                                End If
                                strReport = strReport & "NEW Vehicle (" & intCount.ToString & ")" & dblVehicle.ToString & vbNewLine & vbNewLine
                            ...
                            Try
                                ...
                                strReport = strReport & vbNewLine & "START Walk prob (" & intCount.ToString & ")" & strValue.ToString & vbNewLine
                                If strValue.Contains("*") Or strValue.Contains("+") Or strValue.Contains("!") Then
                                    If strValue.Contains("*") Then
                                        strValue = strValue.Replace("*", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Walk Prob Array Value *(" & intCount.ToString & ")" & (dblCoeff * Math.Log(dblInputValue)).ToString & vbNewLine
                                        dblProbWalk = dblProbWalk + (dblCoeff * Math.Log(dblInputValue))
                                    ElseIf strValue.Contains("+") Then
                                        strValue = strValue.Replace("+", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Walk prob Array Value +(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue / 1000)).ToString & vbNewLine
                                        dblProbWalk = dblProbWalk + (dblCoeff * (dblInputValue / 1000))
                                    ElseIf strValue.Contains("!") Then
                                        strValue = strValue.Replace("!", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Walk prob Array Value !(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue * 100)).ToString & vbNewLine
                                        dblProbWalk = dblProbWalk + (dblCoeff * (dblInputValue * 100))
                                    End If
                                Else
                                    dblCoeff = CDbl(strValue)
                                    dblProbWalk = dblProbWalk + (dblCoeff * dblInputValue)
                                End If
                                strReport = strReport & "NEW Walk Prob(" & intCount.ToString & ")" & dblProbWalk.ToString & vbNewLine & vbNewLine
                            ...
                            Try
                                ...
                                strReport = strReport & vbNewLine & "START Walk (" & intCount.ToString & ")" & strValue.ToString & vbNewLine
                                If strValue.Contains("*") Or strValue.Contains("+") Or strValue.Contains("!") Then
                                    If strValue.Contains("*") Then
                                        strValue = strValue.Replace("*", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Walk Array Value *(" & intCount.ToString & ")" & (dblCoeff * Math.Log(dblInputValue)).ToString & vbNewLine
                                        dblWalk = dblWalk + (dblCoeff * Math.Log(dblInputValue))
                                    ElseIf strValue.Contains("+") Then
                                        strValue = strValue.Replace("+", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Walk Array Value +(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue / 1000)).ToString & vbNewLine
                                        dblWalk = dblWalk + (dblCoeff * (dblInputValue / 1000))
                                    ElseIf strValue.Contains("!") Then
                                        strValue = strValue.Replace("!", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Walk Array Value !(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue * 100)).ToString & vbNewLine
                                        dblWalk = dblWalk + (dblCoeff * (dblInputValue * 100))
                                    End If
                                Else
                                    dblCoeff = CDbl(strValue)
                                    dblWalk = dblWalk + (dblCoeff * dblInputValue)
                                End If
                                strReport = strReport & "NEW Walk (" & intCount.ToString & ")" & dblWalk.ToString & vbNewLine & vbNewLine
                            ...
                            Try
                                ...
                                strReport = strReport & vbNewLine & "START Bike prob (" & intCount.ToString & ")" & strValue.ToString & vbNewLine
                                If strValue.Contains("*") Or strValue.Contains("+") Or strValue.Contains("!") Then
                                    If strValue.Contains("*") Then
                                        strValue = strValue.Replace("*", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Bike prob Array Value *(" & intCount.ToString & ")" & (dblCoeff * Math.Log(dblInputValue)).ToString & vbNewLine
                                        dblProbBike = dblProbBike + (dblCoeff * Math.Log(dblInputValue))
                                    ElseIf strValue.Contains("+") Then
                                        strValue = strValue.Replace("+", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Bike prob Array Value +(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue / 1000)).ToString & vbNewLine
                                        dblProbBike = dblProbBike + (dblCoeff * (dblInputValue / 1000))
                                    ElseIf strValue.Contains("!") Then
                                        strValue = strValue.Replace("!", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Bike prob Array Value !(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue * 100)).ToString & vbNewLine
                                        dblProbBike = dblProbBike + (dblCoeff * (dblInputValue * 100))
                                    End If
                                Else
                                    dblCoeff = CDbl(strValue)
                                    dblProbBike = dblProbBike + (dblCoeff * dblInputValue)
                                End If
                                strReport = strReport & "NEW Bike Prob (" & intCount.ToString & ")" & dblProbBike.ToString & vbNewLine & vbNewLine
                            ...
                            Try
                                ...
                                strReport = strReport & vbNewLine & "START Bike (" & intCount.ToString & ")" & strValue.ToString & vbNewLine
                                If strValue.Contains("*") Or strValue.Contains("+") Or strValue.Contains("!") Then
                                    If strValue.Contains("*") Then
                                        strValue = strValue.Replace("*", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Bike Array Value *(" & intCount.ToString & ")" & (dblCoeff * Math.Log(dblInputValue)).ToString & vbNewLine
                                        dblBike = dblBike + (dblCoeff * Math.Log(dblInputValue))
                                    ElseIf strValue.Contains("+") Then
                                        strValue = strValue.Replace("+", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Bike Array Value +(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue / 1000)).ToString & vbNewLine
                                        dblBike = dblBike + (dblCoeff * (dblInputValue / 1000))
                                    ElseIf strValue.Contains("!") Then
                                        strValue = strValue.Replace("!", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Bike Array Value !(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue * 100)).ToString & vbNewLine
                                        dblBike = dblBike + (dblCoeff * (dblInputValue * 100))
                                    End If
                                Else
                                    dblCoeff = CDbl(strValue)
                                    dblBike = dblBike + (dblCoeff * dblInputValue)
                                End If
                                strReport = strReport & "NEW Bike  (" & intCount.ToString & ")" & dblBike.ToString & vbNewLine & vbNewLine
                            ...
                            Try
                                ...
                                strReport = strReport & vbNewLine & "START Transit prob (" & intCount.ToString & ")" & strValue.ToString & vbNewLine
                                If strValue.Contains("*") Or strValue.Contains("+") Or strValue.Contains("!") Then
                                    If strValue.Contains("*") Then
                                        strValue = strValue.Replace("*", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Transit prob Array Value *(" & intCount.ToString & ")" & (dblCoeff * Math.Log(dblInputValue)).ToString & vbNewLine
                                        dblProbTransit = dblProbTransit + (dblCoeff * Math.Log(dblInputValue))
                                    ElseIf strValue.Contains("+") Then
                                        strValue = strValue.Replace("+", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Transit Array Value +(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue / 1000)).ToString & vbNewLine
                                        dblProbTransit = dblProbTransit + (dblCoeff * (dblInputValue / 1000))
                                    ElseIf strValue.Contains("!") Then
                                        strValue = strValue.Replace("!", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Transit prob Array Value !(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue * 100)).ToString & vbNewLine
                                        dblProbTransit = dblProbTransit + (dblCoeff * (dblInputValue * 100))
                                    End If
                                Else
                                    dblCoeff = CDbl(strValue)
                                    dblProbTransit = dblProbTransit + (dblCoeff * dblInputValue)
                                End If
                                strReport = strReport & "NEW Transit Prob  (" & intCount.ToString & ")" & dblProbTransit.ToString & vbNewLine & vbNewLine
                            ...
                            Try
                                ...
                                strReport = strReport & vbNewLine & "START Transit (" & intCount.ToString & ")" & strValue.ToString & vbNewLine
                                If strValue.Contains("*") Or strValue.Contains("+") Or strValue.Contains("!") Then
                                    If strValue.Contains("*") Then
                                        strValue = strValue.Replace("*", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Transit Array Value *(" & intCount.ToString & ")" & (dblCoeff * Math.Log(dblInputValue)).ToString & vbNewLine
                                    ...
                                    ElseIf strValue.Contains("!") Then
                                        strValue = strValue.Replace("!", "0")
                                        dblCoeff = CDbl(strValue)
                                        strReport = strReport & "Transit Array Value !(" & intCount.ToString & ")" & (dblCoeff * (dblInputValue * 100)).ToString & vbNewLine
                                        dblTransit = dblTransit + (dblCoeff * (dblInputValue * 100))
                                    End If
                                Else
                                    dblCoeff = CDbl(strValue)
                                    dblTransit = dblTransit + (dblCoeff * dblInputValue)
                                End If
                                strReport = strReport & "NEW Transit (" & intCount.ToString & ")" & dblTransit.ToString & vbNewLine & vbNewLine
                        ...
                        dblVMT = Exp(dblVMT) * dblVMTSacleFactor '1.505  ************Need variable from user inout here*******************
                        ...
                        'HU CALCS
                        If intHUFld >= 0 Then
                            dblHH = pFeat.Value(intHUFld)
                            If dblHH > 0 Then
                                dblHU_VMT = dblVMT * dblHH
                                dblHU_Vehicle = dblVehicle * dblHH
                                dblHU_Walk = dblWalk * dblHH
                                dblHU_Bike = dblBike * dblHH
                                dblHU_Transit = dblTransit * dblHH
                                strReport = strReport & "HU Calc Value:" & dblHH.ToString & vbNewLine
